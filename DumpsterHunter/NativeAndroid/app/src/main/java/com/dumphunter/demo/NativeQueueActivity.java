...
public class NativeQueueActivity extends Activity {
    ...
    private void loadFolders() {
        ...
        File root = new File(base, "DumpReports");
    ...
    private void buildUi() {
        ...
        page.addView(text("WHAT WAS DUMPED?", 15, true), full(dp(30)));
        ...
        LinearLayout property = new LinearLayout(this);
        rightOfWay = whiteSpinner(RIGHT_OF_WAY);
        privateProperty = whiteSpinner(PRIVATE_PROPERTY);
        property.addView(rightOfWay, weighted(dp(48)));
        property.addView(privateProperty, weighted(dp(48)));
    ...
    private void loadCurrent() {
        try {
            ...
            String issue = report.optString("dumpingIssue", report.optString("locationType", ""));
            int selected = Arrays.asList(ISSUES).indexOf(issue);
            category.setSelection(selected < 0 ? 0 : selected);
            int row = Arrays.asList(RIGHT_OF_WAY).indexOf(report.optString("publicRightOfWay", "Yes"));
    ...
    private boolean saveCurrent() {
        ...
        try {
            ...
            if (type.startsWith("[SELECT")) {
                Toast.makeText(this, "Select where the dumping is located.", Toast.LENGTH_LONG).show();
                return false;
            }
    ...
    private void launchOfficialForm() {
        File folder = folders.get(index);
        Intent form = new Intent(this, DumpSubmissionActivity.class);
    ...
    private Spinner whiteSpinner(String[] values) {
        ...
        ArrayAdapter<String> adapter = new ArrayAdapter<String>(this,
            ...
            private View style(View view) {
                TextView label = (TextView)view; label.setTextColor(Color.rgb(20, 25, 30));
                label.setBackgroundColor(Color.WHITE); label.setTextSize(15);
                label.setPadding(dp(12), 0, dp(12), 0); return label;
            }
            @Override public View getView(int position, View convertView, ViewGroup parent) {
                return style(super.getView(position, convertView, parent));
            }
        ...
        adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        spinner.setAdapter(adapter); spinner.setBackgroundColor(Color.WHITE);
        spinner.setPopupBackgroundDrawable(new ColorDrawable(Color.WHITE)); return spinner;
    }
